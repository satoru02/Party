        ...
        <v-menu
          ...
          <template v-slot:activator="{ on, attrs }">
            <div v-bind="attrs" v-on="on">
              <v-badge
                v-if="entryNotifications.length > 0"
                color="red"
                :content="entryNotifications.length"
                offset-y="10"
                offset-x="7">
          ...
          <v-list
            class="overflow-y-auto rounded-s"
            max-height="650"
            ...
            <v-list-item
              class="tile"
              v-for="(notification, index) in entryNotifications"
              :key="index">
              <router-link :to="{ name: 'Notification', params: {id: `${notification.attributes.id }`}}">
                <v-badge
                  ...
                  color="#2176ff">
                  <v-list-item-title
                    v-if="notification.attributes.classification === 'entry'"
                    class="ml-10 mr-10"
                    style="color:#ced4da">
                  あなたのイベントへのエントリーが届きました！
                  </v-list-item-title>
                  <v-list-item-title
                    v-if="notification.attributes.classification === 'entryResponse'"
                    class="ml-10 mr-10"
                    style="color:#ced4da">
                  応募したエントリーの結果が届きました！
                  </v-list-item-title>
