      ...
      <v-col cols="2" style="max-width: 100%;" class="flex-grow-1 flex-shrink-0 mt-7">
        <v-responsive class="overflow-y-auto fill-height" height="550">
          <v-card color="#161a1d" flat class="fill-height">
            ...
            <!-- <realtime-messages :realtime_messages="realtime_messages"></realtime-messages> -->
            </v-card-text>
          </v-card>
        </v-responsive>
...
<script>
  ...
  export default {
    ...
    components: {
      ...
      'avatar': Avatar,
      'realtime-messages': RealtimeMessage,
    },
    data() {
      return {
        room_token: '',
    ...
    watch: {
      '$route': 'createCable',
      '$route': 'getMessage',
      // '$route': channels.RoomChannel.connected()
    },
    methods: {
      createCable(){
