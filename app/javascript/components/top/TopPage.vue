<template>
  <v-row>
    ...
    <v-col cols=12 sm=12 md=6 lg=6 xl=6 :class="[$vuetify.breakpoint.mdAndUp ? 'mt-0': 'mt-n9']">
      <v-row>
        <v-col v-if="$route.name === 'Top'" v-model="date" cols=6 sm=12 md=12 lg=10 xl=10 align="start"
          class="ml-5 mb-n2">
          <div style="font-weight:bold;" :class="[$vuetify.breakpoint.mdAndUp ? 'mt-1 text-h6': 'subtitle-1']">注目のイベントをチェック！</div>
        </v-col>
        <v-col v-if="$route.name === 'TopFilter'" v-model="date" cols=6 sm=12 md=12 lg=10 xl=10 align="start"
          class="ml-5 mb-n2">
          <div style="font-weight:bold;" :class="[$vuetify.breakpoint.mdAndUp ? 'mt-1 text-h6': 'subtitle-1']">{{date}}のイベント</div>
        </v-col>
        <v-col v-if="$route.name === 'Category'" cols=6 sm=12 md=12 lg=10 xl=10 align="start" class="ml-5 mb-n2">
          <div style="font-weight:bold;" :class="[$vuetify.breakpoint.mdAndUp ? 'mt-1 text-h6': 'subtitle-1']">{{this.$route.params.slug}}</div>
        </v-col>
        <v-col v-if="$route.name === 'Search'" cols=6 sm=12 md=12 lg=10 xl=10 align="start" class="ml-5 mb-n2">
          <div style="font-weight:bold;" :class="[$vuetify.breakpoint.mdAndUp ? 'mt-1 text-h6': 'subtitle-1']">検索結果：{{this.$route.params.query}}</div>
        </v-col>
        <v-col v-if="$route.name === 'Tag'" cols=6 sm=12 md=12 lg=10 xl=10 align="start" class="ml-5 mb-n2">
          <div style="font-weight:bold;" :class="[$vuetify.breakpoint.mdAndUp ? 'mt-1 text-h6': 'subtitle-1']">#{{this.$route.params.name}}</div>
        </v-col>
        <v-col cols=2 class="hidden-lg-only hidden-xl-only" />
        <v-col cols=1 lg=1 xl=1 :class="[$vuetify.breakpoint.mdAndUp ? 'mt-2 ml-n11': 'mt-0']">
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item @click="filterSearch(item.identifier)" class="tile" v-for="(item, n) in items" :key="n">
